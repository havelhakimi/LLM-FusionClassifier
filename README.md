...
- For LLaMA2, the embedding dimension is 4096. We average the embeddings across all tokens in the last 5 layers and save the resulting averaged embeddings. For each sample, we store a tensor of shape (5, 4096), where 5 represents the last 5 layers, and 4096 is the embedding dimension. For reference see the log file `Create_data_LLAMA2_GO.out` in `Logs` folder.
