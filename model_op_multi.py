...
def Train_multi(dataloader, device, model, loss_fn, optimizer):
    loss_list, acc_list, micro_f1_list, macro_f1_list ,w_f1_list= [], [], [], [],[]
    ...
    for batch_i, batch_loader in enumerate(tqdm(dataloader)):
        ...
        weighted_f1 = f1_score(batch_y.cpu(), pred_y.cpu(), average='weighted')
        loss = loss.cpu()
    ...
    return np.mean(loss_list),np.mean(acc_list),np.mean(w_f1_list)


def Valid_multi(dataloader, device, model, loss_fn):
    avg_loss = 0
    total_pred, total_y = [], []
    ...
    weighted_f1=f1_score(total_y.cpu(), total_pred.cpu(), average='weighted')
    
    #print(f'avg loss: {avg_loss:.4f}')
    #print(f'acc: {acc:.4f}')
    #print(f'micro_f1: {micro_f1:.4f}')
    #print(f'macro_f1: {macro_f1:.4f}')
    #print(f'f1: {weighted_f1_score:.4f}')

    return avg_loss,acc,weighted_f1
